...
export function Grid({
  ...
  const layouts = {
    ...
    ${items === 3 && 'sm:grid-cols-3'} ${items > 3 && 'md:grid-cols-3'} 
    ${items >= 4 && 'lg:grid-cols-4'}`,
    products: `grid-cols-1 ${items >= 2 && 'md:grid-cols-2'} 
    ${items >= 3 && 'lg:grid-cols-3 '} ${items >= 4 && '2xl:grid-cols-4'}`,
    auto: 'auto-cols-auto',
    blog: 'grid-cols-1 md:grid-cols-2',
  };

  const gaps = {
    default: 'grid gap-2 gap-y-2',
    blog: 'grid gap-2',
  };
